...
for jki in range(0,len(gbn)):
        Hsd = numpy.zeros((len(rpbins)-1,len(zpbins)-1))
        Hsr = numpy.zeros((len(rpbins)-1,len(zpbins)-1))
        Hrdrr = numpy.zeros((len(rpbins)-1,len(zpbins)-1))
        rjklen = numpy.fromfile('/home/wgpdata_errors/' + rdstr2 + '_'+ str(Dlen) + 'jklen5' + '.' + str(jki) + clrstr1 + str_nsubl  + '.' + dirno + '.' + str(mmin) + '.' + typ1 + '.alltracer',dtype=numpy.int)
        ...
        rjklen_12 = numpy.fromfile('/home/wgpdata_errors/' + rdstr2 + '_'+ str(Dlen) + 'jklen5' + '.' + str(0) + '' + str_nsubl  + '.' + dirno + '.' + str(12.0) + '.' + typ1 + '.alltracer',dtype=numpy.int)
        ...
        for sij in range(0,gnp):
                si1 = sij
                sj1 = sij + 1  
                Hsd = Hsd + numpy.fromfile('/home/wgpdata_errors/' + rdstr2 + '_'+ str(Dlen) + 'SpD' + '.' + str(jki) + clrstr1 + str_nsubl + '.' + dirno + '.' + str(mmin) + '.' + str(si1) + str(sj1) + typ1 + '.alltracer',dtype = (numpy.float,len(rpbins)-1))
                Hsr = Hsr + numpy.fromfile('/home/wgpdata_errors/' + rdstr2 + '_'+ str(Dlen) + 'SpR' + '.' + str(jki) + clrstr1 + str_nsubl + '.' + dirno + '.' + str(mmin) + '.' + str(si1) + str(sj1) + typ1 + '.alltracer',dtype = (numpy.float,len(rpbins)-1))
                Hrdrr = Hrdrr + numpy.fromfile('/home/wgpdata_errors/' + rdstr2 + '_'+ str(Dlen) + 'RdRr' + '.' + str(0) + '' + str_nsubl + '.' + dirno + '.' + str(12.0) + '.' + str(si1) + str(sj1) + typ1 + '.alltracer',dtype = (numpy.float,len(rpbins)-1))
                
        Hsr = Hsr*(rDlen2/rDlen3)
        Hrdrr = (Hrdrr*rDlen1*rDlen3)/(rDlen4_12*rDlen3_12)          
        for ari in range(0,len(rpbins)-1):    
                for rj in range(0,len(zpbins)-1):
                       galPcorrpoints_jk[jki,ari] = galPcorrpoints_jk[jki,ari] + (((Hsd[ari,rj:rj+1]).sum() - (Hsr[ari,rj:rj+1]).sum())*(2*diff(zpbins)[0]))/((Hrdrr[ari,rj:rj+1]).sum())
        print jki
rpoints = np.array(rpoints)/1000.0
galPcorrpoints_jk = (1.0/(2.0*etaR*bgf))*np.array(galPcorrpoints_jk)/1000.0
...
matplotlib.pyplot.errorbar(rpoints[:],galPcorrpoints[:],yerr =errvals_jk[:],color = 'b',lw = 2.0) 
